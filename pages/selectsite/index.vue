		...
		<view class="taps_List">
			...
			<view class="taps_Lis" v-for="(item,index) in dataList" :key="index" @click="tapsChange(item)">
				<view class="taps_Lis_img">
					<image :src="item.attList.length > 0 ? item.attList[0].uaPath : '../../static/images/tx.png' "
						mode=""></image>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			tapsChange(val) {
				console.log("val", val)
				// url: `selectsite/item?id=${val.id}&type=${val['类型']}`,
				uni.navigateTo({
					url: `../selectsite/item?id=${val.id}&type=${val.type}&title=${val.title}&pollutionType=${val.pollutionType}`,
				});
			},
			// 初始化数据列表
			async initDataList() {
				...
				let res = await API.DevOpsList({
					type: this.tabType,
					findKey:this.keyword
				});
